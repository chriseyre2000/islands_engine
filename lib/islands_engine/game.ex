defmodule IslandsEngine.Game do 
  ...
  def handle_call({:guess_coordinate, player_key, row, col}, _from, state_data) do
    ...
    oppenent_board = player_board(state_data, opponent_key)
    with {:ok, rules} <- 
           Rules.check(state_data.rules,{:guess_coordinate, player_key}),  
         {:ok, coordinate} <- 
           Coordinate.new(row, col),
         {hit_or_miss, forested_island, win_status, oppenent_board, coordinate} <-
           Board.guess(oppenent_board, coordinate),
         {:ok, rules} <-
           Rules.check(rules, {:win_check, win_status})
    do
      state_data
      |> update_board(opponent_key, oppenent_board)
